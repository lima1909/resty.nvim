...
describe("curl-parser:", function()
	it("only url, no method", function()
		...
		assert.are.same(r.meta.curl, { starts = 1, ends = 1 })
	end)
	...
	it("multi-line input", function()
		...
		assert.are.same(r.meta.curl, { starts = 1, ends = 2 })
	end)
	...
	it("with headers", function()
		...
		assert.are.same(r.meta.curl, { starts = 1, ends = 4 })
		...
		assert.are.same(r.meta.curl, { starts = 1, ends = 3 })
	end)
